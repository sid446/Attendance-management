...
/** Requests can carry a few hundred employees plus a full replay, so allow headroom. */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Names normalise to a dots/spaces/case-insensitive key, so "Arif.Khan" and
    // "arif khan" both resolve to the same employee.
    const byNameKey = new Map<string, Array<{ id: string; name: string; bf: number }>>();
    for (const u of users) {
      ...
      byNameKey.get(key)!.push({
        id: String(u._id),
        name: String(u.name || ''),
        bf: Number(u.leaveBalance?.balanceAsOfJan26 || 0),
      });
    }
    ...
    for (const row of rows) {
      ...
      if (!Number.isFinite(balance) || balance < 0) {
        invalid.push({ excelName, reason: 'Leaves B/F is not a valid number' });
        continue;
      }
      ...
      matched.push({
        ...
        currentBalanceAsOfJan26: target.bf,
        newBalanceAsOfJan26: balance,
      });
    }
    ...
    const openingBalanceOverrides = Object.fromEntries(
      matched.map((m) => [m.userId, m.newBalanceAsOfJan26])
    );
    ...
    for (const m of matched) {
      await User.findByIdAndUpdate(m.userId, {
        $set: {
          'leaveBalance.balanceAsOfJan26': m.newBalanceAsOfJan26,
    ...
    console.error('Leave opening balance upload error:', error);
    return NextResponse.json(
      {
        success: false,
